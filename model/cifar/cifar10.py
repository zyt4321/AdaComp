...
def _variable_on_cpu(name, shape, initializer):
    ...
    with tf.device('/cpu:0'):
        ...
        var = tf.reshape(tf.get_variable(name, [total], initializer=initializer, collections=["W_global"]), shape)
    return var
...
def distorted_inputs(data_range=(1, 6)):
...
def loss(logits, labels, batch_size):
    ...
    # indices = tf.reshape(list(range(batch_size.eval())), [batch_size, 1])
