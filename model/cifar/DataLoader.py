...
class Dataloader:
    ...
    def data_augmentation(self, images, mode='train', flip=False,
        ...
        # new
        if brightness:
            images = self._random_brightness(images)
        if contrast:
            images = self._random_contrast(images, lower=0.2, upper=1.8)
        #
        # # 图像白化
        if whiten:
    ...
    def _image_whitening(self, images):
        ...
        #     # mean = numpy.mean(old_image)
        #     mean = numpy.mean(old_image)
        #     var = numpy.mean(numpy.square(old_image)) - numpy.square(mean)
        #     std = numpy.sqrt(var)
        #     std = numpy.max([std,
        #                      1.0 / numpy.sqrt(float(images.shape[1] * images.shape[2] * images.shape[3]))])
        #     new_image = (old_image - mean) / std
        #     images[i, :, :, :] = new_image
        return images
    ...
    def _random_brightness(self, images, delta=63):
        for i in range(images.shape[0]):
            old_image = images[i, :, :, :]
            new_image = old_image
    ...
    def _random_contrast(self, images, lower=0.2, upper=1.8):
        for i in range(images.shape[0]):
            old_image = images[i, :, :, :]
            new_image = old_image
            d = random.uniform(lower, upper)

            new_image = new_image.reshape((-1, images.shape[-1]))
            mean = numpy.mean(new_image, axis=0, keepdims=True)
            new_image = (new_image - mean) * d + mean
            new_image = new_image.reshape(images.shape[1:])
            images[i, :, :, :] = new_image
